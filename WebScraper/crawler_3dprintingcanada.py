...
def fetch_data_from_page(url):
    ...
    products = soup.find_all('div', class_='pb-10')  # Adjust class name based on site structure
    product_data = []

    #iterate
    for product in products:

        title_element = product.find('h4', class_='text-sm group-hover:text-red line-clamp-2 flex-shrink-0') 
        title = title_element.text.strip() if title_element else "Unknown Title"

        price_element = product.find('span', class_='mt-2')
...
def scrape_all_pages(base_url):
    ...
    while True:
        ...
        if not data:
            break  # If no more products, exit the loop

        all_product_data.extend(data)

        # Check if there is a next page
        next_page = soup.find('a', {'class': 'block'})  # Adjust based on site's next page button/link
...
# Base URL
base_url = 'https://www.3dprintingcanada.com'
